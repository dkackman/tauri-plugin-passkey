...
- **Re-sync shared tooling from SE (these originated in SE):** the Apple
  signing/provisioning scripts (`build-macos-dev.sh`, `sign_app.sh`,
  `setup-dev.sh`) and `scripts/test-macos.sh` began in SE and were copied into
  webauthn; SE's copies have since matured. Bring webauthn's versions into
  alignment with SE's **current** versions (diff SE → webauthn, port SE's
  improvements, keep only webauthn-specific deltas that are genuinely required by
  the webauthn/passkey domain — e.g. different entitlements or associated-domain
  handling). Do not freeze the fork's older copies.
- **Keep webauthn-original assets (do not regress):** the production-grade
  `macos/README.md` and `ios/README.md` with troubleshooting tables, and the
  security-remediation `validation.rs` (rpId/origin + PRF salt validation). These
  are webauthn-specific and have no SE counterpart to reconcile against.
...
- Shared Apple/toolchain scripts match SE's current versions (webauthn-specific
  deltas justified); webauthn-original assets (platform READMEs, `validation.rs`)
  remain intact.
